const slugify = (value) => value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
const city = (name, highlight = "Rooms, PGs, flats and local rental demand") => ({
    name,
    slug: slugify(name),
    highlight
});
const locality = (name, city, district, highlight) => ({
    name,
    slug: slugify(name),
    city,
...
export const majorCitiesByDistrict = {
    Ahilyanagar: [
...
export const cityPages = Object.entries(majorCitiesByDistrict).flatMap(([district, cities]) => cities.map((item) => ({
    city: item.name,
    district,
    slug: item.slug,
